...
from src.services.duplicate_detector import find_duplicate_applications
...
def get_candidate_duplicates(
    ...
    duplicates = find_duplicate_applications(
        db, current_user.id, candidate.email, candidate.job_id, exclude_candidate_id=candidate.id
    )
    return {"duplicate_applications": duplicates}
